...
`todo.md`, close the loops, record what got done onto today's daily log, decide tomorrow's
shape, then help me empty my head so I can stop thinking about work and go about my evening.

`/daily` plans the day and creates `todo.md`; I update `todo.md` all day; `/shutdown`
...
- For each **checked** `[x]` item in `todo.md`:
  ...
  - Complete the corresponding **Todoist** task (find by content in that project; use
    `complete-tasks`). Never auto-complete anything that isn't checked in `todo.md`.
  - Log it in today's daily `## 📝 Work Log`.
- For each item in `todo.md`'s `## Captured` section: process like `/capture` — route real
...
- Fill today's `## 📝 Work Log` (what happened) and `## 🔄 Reflection` (wins, what slipped
  ...
  reads later. Infer what got done from `todo.md` + completions; if a Top-3 didn't land,
  say so and why.

### 4. Decide tomorrow
- Carry every **unchecked** `todo.md` item forward (leave it open in its project/Todoist so
...
- Confirm `todo.md` is reconciled and the day is logged. Then I'm done — go about the evening.
